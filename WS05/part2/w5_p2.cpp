...
int main(int argc, char** argv)
{
	...
	if (argc > 2) {
		...
		if (!movieFile.is_open()) {
			std::cerr << "ERROR: Cannot open movie file.\n";
			exit(AppErrors::CannotOpenFile);
		}

		std::string movieLine;
		int movieIndex = 0;
		while (std::getline(movieFile, movieLine) && movieIndex < 5) {
			if (!movieLine.empty() && movieLine[0] != '#') {
				movies[movieIndex] = seneca::Movie(movieLine);
				movieIndex++;
			}
		}
